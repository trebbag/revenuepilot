...
import { ChevronLeft, ChevronRight, Check, Circle, AlertCircle, AlertTriangle, Target, Lightbulb, Settings, Eye, EyeOff, Filter, Heart, FileText, Users, ClipboardCheck, Highlighter, HelpCircle, MessageSquare, TrendingUp, Shield, Zap, Code, Activity, User } from 'lucide-react';
...
export function StepContent({ step, onNext, onPrevious, onActiveItemChange, onShowEvidence, patientQuestions = [], onUpdatePatientQuestions, showPatientTray: externalShowPatientTray, onShowPatientTray, onInsertToNote }: StepContentProps) {
  ...
  const updateItemStatus = (itemId: number, status: Item['status']) => {
    setItems(prev => {
      ...
      return prev.map(item => 
        item && item.id === itemId ? { ...item, status } : item
      );
    });
  };
        ...
        >
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-center text-xs">
              ...
              <span className="text-slate-500">{items.filter(item => item && item.status === 'completed').length}/{items.length}</span>
            </div>
                  ...
                  return (
                    ...
                    >
                      <Card className={`
                        ...
                        ${isActive 
                          ? 'shadow-2xl shadow-slate-900/20 border-slate-300/60 bg-white' 
                          : 'shadow-lg shadow-slate-900/15 hover:shadow-xl hover:shadow-slate-900/25'
                        }
                        transition-all duration-300
                      `}>
                        {/* Enhanced left edge indicator */}
                        <div className={`absolute top-0 left-0 bottom-0 w-1 ${
                          item.category === 'ICD-10' ? 'bg-blue-500' :
                          item.category === 'CPT' ? 'bg-green-500' :
                          item.category === 'Public Health' ? 'bg-purple-500' :
                          'bg-slate-500'
                        }`} />
                        ...
                        <div className="relative z-10 h-full p-5 flex flex-col">
                          ...
                          <div className="flex items-start justify-between mb-3">
                            <div className="flex items-center gap-3">
                              <div className={`w-8 h-8 rounded-lg flex items-center justify-center shadow-sm ${
                                item.category === 'ICD-10' ? 'bg-blue-100 text-blue-600' :
                                item.category === 'CPT' ? 'bg-green-100 text-green-600' :
                                item.category === 'Public Health' ? 'bg-purple-100 text-purple-600' :
                                'bg-slate-100 text-slate-600'
                              }`}>
                                {step.stepType === 'selected' ? <Shield size={14} /> :
                                 step.stepType === 'suggested' ? <Zap size={14} /> :
                              ...
                              <div className="flex flex-col gap-0.5">
                                <div className={`text-xs font-bold leading-none ${
                                  item.category === 'ICD-10' ? 'text-blue-700' :
                                  item.category === 'CPT' ? 'text-green-700' :
                                  item.category === 'Public Health' ? 'text-purple-700' :
                                  'text-slate-700'
                                }`}>
                                  {item.category}
                                </div>
                                <div className={`text-xs font-medium leading-none ${
                                  step.stepType === 'selected' ? 'text-emerald-600' :
                                  step.stepType === 'suggested' ? 'text-violet-600' :
                                  'text-slate-500'
                                }`}>
                                  {step.stepType === 'selected' ? 'Selected' :
                                   step.stepType === 'suggested' ? 'AI Suggested' :
                                   'Review Item'}
                                </div>
                              </div>
                            </div>
                              ...
                              {(step.id === 1 || step.id === 2) && (item as any).confidence && (
                                <div className="flex items-center gap-2">
                                  ...
                                  <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold shadow-sm ${
                                    (item as any).confidence >= 90 ? 'bg-emerald-100 text-emerald-700' :
                                    (item as any).confidence >= 75 ? 'bg-amber-100 text-amber-700' :
                          ...
                          <div className="mb-3" style={{ minHeight: '2.8rem' }}>
                            <h4 className="font-semibold text-slate-900 text-sm leading-[1.35] line-clamp-2">
                              {item.title || 'Untitled'}
                            </h4>
                          </div>
                          ...
                          <div className="flex-1 mb-3" style={{ minHeight: '2.4rem' }}>
                            <p className="text-xs text-slate-600 leading-[1.4] line-clamp-2 h-full flex items-start">
                            ...
                            <div className="flex items-center gap-2.5 text-xs">
                              ...
                              {step.stepType === 'selected' && (item as any).docSupport && (
                                <div className={`flex items-center gap-1.5 ${
                                  (item as any).docSupport === 'strong' ? 'text-emerald-600' :
                                  (item as any).docSupport === 'moderate' ? 'text-amber-600' :
                                  'text-red-600'
                                }`}>
                                  <Activity size={11} />
            ...
            >
              ...
              <div className="flex-shrink-0 p-4 pb-2.5 border-b border-slate-150">
                <div className="flex items-start gap-3">
                  <div className="flex-shrink-0">
                    <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${getCategoryColors(activeItem.category)}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      >
                        ...
                        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1.5 px-2 py-1 bg-slate-800/90 text-white text-xs rounded whitespace-nowrap shadow-md pointer-events-none z-30 opacity-0 group-hover:opacity-100 transition-opacity duration-200 backdrop-blur-sm">
                          ...
                          <div className="absolute top-full left-1/2 -translate-x-1/2" style={{ 
                            width: 0, 
                            height: 0, 
                    ...
                    <div className="flex items-center gap-3 text-xs">
                      ...
                      <span className="text-slate-500 text-xs">
                        {step.id === 0 ? 'Documentation Gap' : 
                         step.id === 0 ? 'Current Code' : 
                         step.id === 2 ? 'AI Suggestion' : 'Review Item'}
                      </span>
                    </div>
                  </div>
              ...
              <div 
                className={`overflow-y-auto px-4 ${
                  (step.id === 1 || step.id === 2) 
                    ? 'code-step-scroll pb-24' 
                    : 'scrollbar-thin scrollbar-thumb-slate-300 scrollbar-track-transparent hover:scrollbar-thumb-slate-400 pb-4'
                }`}
                style={{ 
                        ...
                        <div className="flex-1">
                          ...
                          <div className="grid grid-cols-2 gap-3 mb-3">
                            <div className="bg-slate-50 rounded-lg p-3">
                              ...
                              <div className={`font-semibold ${(activeItem as any).stillValid ? 'text-emerald-600' : 'text-red-600'}`}>
                                {(activeItem as any).stillValid ? 'Yes' : 'Needs Review'}
                              </div>
                            </div>
                            <div className="bg-slate-50 rounded-lg p-3">
                              <div className="text-xs text-slate-500 mb-1">AI Confidence</div>
                              <div className={`font-semibold ${
                                (activeItem as any).confidence >= 90 ? 'text-emerald-600' :
                                (activeItem as any).confidence >= 75 ? 'text-amber-600' : 'text-red-600'
                              }`}>
                          ...
                          <div className="bg-slate-50 rounded-lg p-3 mb-3">
                            ...
                            <div className={`font-semibold capitalize ${
                              (activeItem as any).docSupport === 'strong' ? 'text-emerald-600' :
                              (activeItem as any).docSupport === 'moderate' ? 'text-amber-600' : 'text-red-600'
                            }`}>
                              ...
                              {(activeItem as any).gaps.map((gap: string, index: number) => (
                                <div key={index} className="bg-amber-50 border border-amber-200 rounded-lg p-3">
                                  ...
                                  <div className="text-xs text-amber-600">
                                    Consider asking patient for clarification during encounter
                                  </div>
                                </div>
                              ))}
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                              <div className="bg-green-50 rounded-lg p-3">
                                ...
                                <div className="font-semibold text-green-700">
                                  {(activeItem as any).codeType === 'CPT' ? 'Billable' : 'Diagnostic'}
                                </div>
                              ...
                              <div className="bg-blue-50 rounded-lg p-3">
                                ...
                                <div className="font-semibold text-blue-700">
                                  {(activeItem as any).confidence >= 90 ? 'Low' :
                                   (activeItem as any).confidence >= 75 ? 'Medium' : 'High'}
                                </div>
                              ...
                              <div className="space-y-1">
                                {(activeItem as any).evidence?.map((evidence: string, index: number) => (
                                  ...
                                  <div className="text-xs text-purple-700">
                                    Multiple supporting elements found in clinical note
                                  </div>
                                )}
                              </div>
                            </div>
                              ...
                              <div className="grid grid-cols-3 gap-2 mt-2">
                                <div className="text-center">
                                  ...
                                  <div className="font-semibold text-indigo-800">
                                    {(activeItem as any).confidence >= 90 ? 'High' :
                                     (activeItem as any).confidence >= 75 ? 'Good' : 'Fair'}
                                  </div>
                                </div>
                                <div className="text-center">
                                  <div className="text-xs text-indigo-600">Accuracy</div>
                                  <div className="font-semibold text-indigo-800">
                                    {(activeItem as any).docSupport === 'strong' ? 'High' :
                                     (activeItem as any).docSupport === 'moderate' ? 'Good' : 'Fair'}
                                  </div>
                                </div>
                                <div className="text-center">
                                  <div className="text-xs text-indigo-600">Completeness</div>
                                  <div className="font-semibold text-indigo-800">
                                    {!(activeItem as any).gaps?.length ? 'Complete' : 'Partial'}
                                  </div>
                                </div>
                              </div>
                            </div>
                          ...
                          <div className="space-y-3">
                            <div className="bg-rose-50 border border-rose-200 rounded-lg p-3">
                              ...
                              <div className="text-xs text-rose-700 leading-relaxed">
                                This diagnostic code enhances care coordination by providing clear communication to consulting physicians, specialists, and other healthcare team members about the patient's documented conditions and risk factors.
                              </div>
                            </div>
                        ...
                        <div className="flex-1">
                          ...
                          <div className="bg-violet-50 border border-violet-200 rounded-lg p-3 mb-3">
                            ...
                            <div className="text-xs text-violet-600 font-medium">
                              Recommendation Strength: {(activeItem as any).confidence >= 90 ? 'Very High' :
                               (activeItem as any).confidence >= 75 ? 'High' : 'Moderate'}
                            </div>
                          </div>
                          <div className="grid grid-cols-2 gap-3 mb-3">
                            <div className="bg-slate-50 rounded-lg p-3">
                            ...
                            <div className="bg-slate-50 rounded-lg p-3">
                              ...
                              <div className="font-semibold text-violet-600 text-sm">
                                {(activeItem as any).suggestedBy}
                              </div>
                            </div>
                          </div>
                          <div className="bg-purple-50 border border-purple-200 rounded-lg p-3">
                          ...
                          <div className="space-y-3">
                            <div className="bg-green-50 border border-green-200 rounded-lg p-3">
                              ...
                              <div className="text-xs text-green-700 leading-relaxed">
                                {(activeItem as any).category === 'diagnosis' ? 'Improves diagnostic specificity and care planning by providing more precise condition documentation that guides treatment decisions and specialist referrals.' :
                                 (activeItem as any).category === 'screening' ? 'Supports preventive care and risk assessment protocols while enabling population health management and quality measure reporting.' :
                                 (activeItem as any).category === 'procedure' ? 'Ensures proper procedure billing and tracking while supporting quality metrics for procedural outcomes and follow-up care coordination.' :
                                 (activeItem as any).category === 'evaluation' ? 'Accurately reflects the complexity of medical decision-making and time invested in patient care evaluation and management.' :
                            ...
                            <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-3">
                              ...
                              <div className="text-xs text-emerald-700 leading-relaxed">
                                {(activeItem as any).codeType === 'CPT' ? 'Captures billable procedures and services that might otherwise go uncompensated, improving practice revenue while ensuring accurate documentation of services provided.' :
                            ...
                            <div className="bg-teal-50 border border-teal-200 rounded-lg p-3">
                              ...
                              <div className="text-xs text-teal-700 leading-relaxed">
                                Supports quality reporting requirements, population health initiatives, and risk adjustment models used by payers and quality organizations for performance measurement and benchmarking.
                              </div>
                            </div>
                          </div>
                        </div>
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="space-y-1.5">
                            ...
                            <p className="text-slate-600 leading-snug text-sm italic">
                              {activeItem.what || 'No information available'}
                            </p>
                          </div>
                        </div>
                      </div>
                  ...
                  <div className="flex-1 p-3 text-xs text-slate-500 space-y-2">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span className="font-medium text-emerald-600">
                        {items.filter(item => item && item.status === 'completed').length}
                      </span>
                    </div>
                    {expandedSection && (
                      <button
                    ...
                    <div className="flex items-center gap-2">
                      {expandedSection && (
                        <span className={`text-xs px-3 py-1 rounded-full font-medium ${
                          expandedSection === 'priority' ? 'bg-red-100 text-red-700' :
                          expandedSection === 'category' ? 'bg-blue-100 text-blue-700' :
                          'bg-emerald-100 text-emerald-700'
                        }`}>
                          {expandedSection}
                        </span>
                          ...
                          >
                            <div className="flex items-center gap-3 px-4 py-3 bg-gradient-to-r from-slate-50/80 to-white border-b border-slate-200/30">
                              <div className={`w-3 h-3 rounded-full ${
                                expandedSection === 'priority' ? (
                                  subcategory === 'high' ? 'bg-red-400' :
                                  subcategory === 'medium' ? 'bg-amber-400' :
                                  'bg-green-400'
                                ) : expandedSection === 'category' ? (
                                  subcategory === 'ICD-10' ? 'bg-blue-400' :
                                  subcategory === 'CPT' ? 'bg-green-400' :
                                  subcategory === 'Public Health' ? 'bg-purple-400' :
                                  'bg-slate-400'
                                ) : (
                                  subcategory === 'completed' ? 'bg-emerald-400' :
                                  subcategory === 'in-progress' ? 'bg-amber-400' :
                                  'bg-slate-400'
                                )
                              }`} />
                              ...
                              {subcategoryItems.map((item, index) => (
                                ...
                                >
                                  <div className="flex items-start gap-3">
                                    ...
                                    <div className="flex-1 min-w-0">
                                      <h6 className="font-medium text-sm text-slate-800 mb-1 line-clamp-1">
                                        {item.title}
                                      </h6>
                          ...
                          {filteredItems.map((item, index) => (
                            ...
                            >
                              <div className="flex items-start gap-3">
                                ...
                                <div className="flex-1 min-w-0">
                                  <h6 className="font-medium text-sm text-slate-800 mb-2 line-clamp-1">
                                    {item.title}
                                  </h6>
