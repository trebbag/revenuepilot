...
vi.mock('react-i18next', () => ({ useTranslation: () => ({ t: (k) => k, i18n: { changeLanguage: () => {} } }) }));
vi.mock('../../api.js', () => ({ pingBackend: vi.fn().mockResolvedValue(true), login: vi.fn(), register: vi.fn(), resetPassword: vi.fn() }));
...
describe('Login interactions', () => {
  it('toggles password visibility', async () => {
    ...
    const toggle = screen.getByRole('button', { name: /toggle password visibility/i }) || screen.getByText(/show/i);
  ...
  it('shows password strength bars on register', async () => {
    render(<Login onLoggedIn={() => {}} />);
    const createBtn = screen.getByRole('button', { name: 'login.register' }) || screen.getByText(/create account/i);
    fireEvent.click(createBtn);
    const pwd = screen.getByLabelText('login.password');
    fireEvent.change(pwd, { target: { value: 'P@s5word123' } });
    expect(await screen.findByText(/login.passwordStrong|login.passwordVeryStrong|Strong|Very strong|Good/)).toBeTruthy();
    const bars = document.querySelectorAll('.strength-bar.active');
    expect(bars.length).toBeGreaterThan(0);
  });

  it('remembers username when checkbox checked', async () => {
    login.mockResolvedValue({ token: 'tok', refreshToken: 'r', settings: { lang: 'en' } });
    ...
    const loginBtn = screen.getByRole('button', { name: 'login.login' }) || screen.getByText(/sign in/i);
