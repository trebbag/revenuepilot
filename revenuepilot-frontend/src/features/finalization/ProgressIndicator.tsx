...
export function ProgressIndicator({ steps, currentStep, onStepClick }: ProgressIndicatorProps) {
  const getStepProgress = () => {
    ...
    const highestCompletedIndex = normalizedStatuses.reduce((acc, status, index) => (status === "completed" ? Math.max(acc, index) : acc), -1)
    const firstActiveIndex = normalizedStatuses.findIndex((status) => status === "in-progress" || status === "blocked")
  ...
  return (
    <div className="bg-white/95 backdrop-blur-lg border-b border-slate-200/50 shadow-sm">
      <div className="w-full px-12 py-8 flex items-center justify-center min-h-0">
        <motion.div initial={{ opacity: 0, y: -15 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-16 w-full">
          ...
          <div className="flex items-center gap-4 flex-shrink-0">
            ...
            <div>
              <h1 className="text-2xl font-bold bg-gradient-to-r from-slate-800 to-blue-700 bg-clip-text text-transparent">Finalization Wizard</h1>
          ...
          <div className="relative flex-1 min-w-0" style={{ height: "80px" }}>
            ...
            <div className="absolute top-1/2 -translate-y-1/2 w-full px-8 flex justify-between items-center z-10">
              {steps.map((step, index) => {
                const status: StepStatus = step.status ? step.status : step.id < currentStep ? "completed" : step.id === currentStep ? "in-progress" : "pending"
                const isCompleted = status === "completed"
                const isInProgress = status === "in-progress"
                const isBlocked = status === "blocked"

                return (
                  <motion.div key={step.id} className="flex flex-col items-center relative" initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.07 }}>
                    <motion.button
                      ...
                      animate={
                        isInProgress
                          ? {
                              boxShadow: ["0 6px 20px rgba(59, 130, 246, 0.25)", "0 10px 30px rgba(59, 130, 246, 0.35)", "0 6px 20px rgba(59, 130, 246, 0.25)"],
                            }
                          : {}
                      }
                    ...
                    >
                      {isCompleted ? (
                        ...
                        <motion.span className="font-semibold text-sm" initial={{ opacity: 0, scale: 0.5 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: index * 0.07 + 0.2 }}>
                          {step.id}
                        </motion.span>
                      )}
                    </motion.button>
                    ...
                    >
                      <div
                        className={`text-sm font-medium transition-colors duration-300 whitespace-nowrap ${
                          isBlocked ? "text-red-600" : isCompleted ? "text-emerald-600" : isInProgress ? "text-blue-600" : "text-slate-600"
                        }`}
                      >
                        {step.title}
                      </div>
