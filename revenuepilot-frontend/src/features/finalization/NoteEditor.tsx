...
import { Bold, Italic, Underline, List, ListOrdered, AlignLeft, AlignCenter, AlignRight, Undo, Redo, FileText, MessageSquare, User } from "lucide-react"
...
export function NoteEditor({ content, onChange, highlightRanges = [], disabled = false, questionsCount = 0, onShowQuestions, onInsertText }: NoteEditorProps) {
    ...
    return (
      ...
      >
        {parts.map((part, partIndex) => {
          if (!part.isHighlight) {
            return (
              <span key={partIndex} style={{ color: "transparent" }}>
                {part.text}
              </span>
            )
          }
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <motion.div initial={{ y: -10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="bg-white border-b border-slate-200/50 px-4 py-4">
        ...
        <div className="flex items-center gap-3 mb-3">
          ...
          <div className="flex-1">
            <h3 className="font-semibold text-slate-900 mb-1">John Smith</h3>
          ...
          {questionsCount > 0 && onShowQuestions && (
            ...
            >
              ...
              Patient Questions
              <div className="w-5 h-5 bg-amber-500 text-white rounded-full flex items-center justify-center text-xs font-bold ml-2">{questionsCount}</div>
            </Button>
          )}
        </div>
        ...
        <div className="flex items-center gap-1 flex-wrap">
          ...
          <div className="flex items-center gap-0.5">
            <Button variant="ghost" size="sm" className="h-9 w-9 p-0 hover:bg-slate-50 text-slate-800 hover:text-slate-900" onClick={() => formatSelection("**", "**")} title="Bold">
              <Bold size={16} />
            </Button>
            <Button variant="ghost" size="sm" className="h-9 w-9 p-0 hover:bg-slate-50 text-slate-800 hover:text-slate-900" onClick={() => formatSelection("*", "*")} title="Italic">
              <Italic size={16} />
            </Button>
            <Button variant="ghost" size="sm" className="h-9 w-9 p-0 hover:bg-slate-50 text-slate-800 hover:text-slate-900" onClick={() => formatSelection("_", "_")} title="Underline">
              <Underline size={16} />
            </Button>
          </div>
          ...
          <div className="flex items-center gap-0.5">
            <Button variant="ghost" size="sm" className="h-9 w-9 p-0 hover:bg-slate-50 text-slate-800 hover:text-slate-900" onClick={() => insertText("\n• ")} title="Bullet List">
              <List size={16} />
            </Button>
          ...
          <div className="ml-auto flex items-center gap-0.5">
            <Button variant="ghost" size="sm" className="h-9 w-9 p-0 hover:bg-slate-50 text-slate-800 hover:text-slate-900" title="Undo">
              <Undo size={16} />
            </Button>
            <Button variant="ghost" size="sm" className="h-9 w-9 p-0 hover:bg-slate-50 text-slate-800 hover:text-slate-900" title="Redo">
              <Redo size={16} />
            </Button>
          </div>
        </div>
      ...
      <motion.div
        ...
        className={`flex-1 relative rounded-lg border transition-all duration-500 overflow-hidden ${hasHighlights ? "border-blue-200/70 shadow-lg shadow-blue-500/10" : "border-slate-200/50"}`}
      >
            ...
            <div className="absolute inset-4">
              <div className="relative w-full h-full">
                ...
                <div className="absolute inset-0 pointer-events-none z-20">
                  <AnimatePresence>{renderTextHighlights()}</AnimatePresence>
                </div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="px-4 py-2 bg-slate-50 border-t border-slate-200/50">
        <div className="flex items-center justify-between text-xs text-slate-500">
          ...
          <div className="text-xs text-slate-500">Characters: {content.length}</div>
        </div>
      </motion.div>
    </div>
  )
