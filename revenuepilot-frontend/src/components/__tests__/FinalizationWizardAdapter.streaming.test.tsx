...
const fetchWithAuthSpy = vi.fn<[RequestInfo | URL, (RequestInit & { json?: boolean; jsonBody?: unknown })?], Promise<Response>>(async (input: RequestInfo | URL) => {
  ...
  it("uses streaming suggestions without triggering REST fallbacks", async () => {
    render(
      <FinalizationWizardAdapter
        {...baseProps}
        streamingCodeSuggestions={[{ id: "live-1", code: "99213", description: "Office visit", receivedAt: Date.now() }]}
