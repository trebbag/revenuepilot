...
import { Sidebar, SidebarContent, SidebarGroup, SidebarGroupContent, SidebarMenu, SidebarMenuItem, useSidebar } from "./ui/sidebar"
  ...
  const content = (
    <motion.div
      ...
      className={`relative flex items-center ${isCollapsed ? "w-12 h-12 p-2 justify-center mx-auto" : "w-full p-3"} rounded-xl transition-all duration-200 cursor-pointer group ${
        isActive ? "bg-sidebar-primary text-sidebar-primary-foreground shadow-md" : "hover:bg-sidebar-accent text-sidebar-foreground hover:text-sidebar-accent-foreground"
      }`}
    ...
    >
      <motion.div className="relative flex items-center justify-center">
        <item.icon className={`w-5 h-5 transition-all duration-200 ${isActive ? "text-sidebar-primary-foreground" : isHovered ? item.accentColor : "text-sidebar-foreground"}`} />
      </motion.div>
        ...
        {!isCollapsed && (
          ...
          >
            <div className="flex flex-col">
              <motion.span className={`font-medium text-sm leading-tight transition-colors duration-200 ${isActive ? "text-sidebar-primary-foreground" : "text-sidebar-foreground"}`} layout="position">
                {item.title}
              </motion.span>
              {isHovered && (
                <motion.span
                  className={`text-xs mt-0.5 leading-tight transition-colors duration-200 ${isActive ? "text-sidebar-primary-foreground/80" : "text-sidebar-foreground/70"}`}
            ...
            <div className="flex items-center gap-2">
                ...
                <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: "spring", stiffness: 500, damping: 30 }}>
                  <Badge
                    ...
                    className={`text-xs px-2 py-0.5 font-medium transition-colors duration-200 ${
                      isActive ? "bg-sidebar-primary-foreground/20 text-sidebar-primary-foreground border-sidebar-primary-foreground/30" : "border-border/50"
                    }`}
                  >
                    {item.badge}
                  </Badge>
              ...
              <motion.div animate={{ x: isHovered ? 3 : 0 }} transition={{ duration: 0.15 }}>
                <ChevronRight className={`w-4 h-4 transition-colors duration-200 ${isActive ? "text-sidebar-primary-foreground/70" : "text-sidebar-foreground/50"}`} />
              </motion.div>
            </div>
          </motion.div>
        )}
...
function NavSection({ title, items, isCollapsed, currentView, onNavigate, onNotificationClick, notificationButtonRef }: NavSectionProps) {
  return (
    <motion.div className="space-y-1" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
      {title && !isCollapsed && (
...
function NavigationSidebarContent({ currentView, onNavigate, currentUser, userDraftCount = 0 }: NavigationSidebarProps) {
  ...
  const applyNotificationsResponse = useCallback((response: NotificationListResponse | null | undefined) => {
    ...
    const mapped: Notification[] = items.map((item) => {
      const timestamp =
        typeof item.timestamp === "string" ? item.timestamp : typeof item.createdAt === "string" ? item.createdAt : typeof item.created_at === "string" ? item.created_at : new Date().toISOString()
      const severity = typeof item.severity === "string" ? item.severity : "info"
      const title = typeof item.title === "string" && item.title.trim().length > 0 ? item.title : "Notification"
      const message = typeof item.message === "string" && item.message.trim().length > 0 ? item.message : "You have a new notification."
      const readAt = typeof item.readAt === "string" ? item.readAt : typeof item.read_at === "string" ? item.read_at : null
      const readFlag = typeof item.isRead === "boolean" ? item.isRead : typeof item.is_read === "boolean" ? item.is_read : undefined
    ...
    const unread = typeof response?.unreadCount === "number" ? response.unreadCount : mapped.reduce((acc, item) => (item.isRead ? acc : acc + 1), 0)
  ...
  const normaliseNotificationPayload = useCallback((payload: Record<string, any>): Notification => {
    const severityCandidate = typeof payload.severity === "string" ? payload.severity : typeof payload.level === "string" ? payload.level : typeof payload.type === "string" ? payload.type : "info"
    ...
    const descriptionCandidate =
      typeof payload.message === "string" ? payload.message : typeof payload.description === "string" ? payload.description : typeof payload.detail === "string" ? payload.detail : undefined

    const message = typeof descriptionCandidate === "string" && descriptionCandidate.trim().length > 0 ? descriptionCandidate : "You have a new notification."

    const idSource = payload.eventId ?? payload.id ?? payload.notificationId ?? Math.random().toString(36).slice(2)

    const readAt = typeof payload.readAt === "string" ? payload.readAt : typeof payload.read_at === "string" ? payload.read_at : undefined

    const readFlag = typeof payload.isRead === "boolean" ? payload.isRead : typeof payload.is_read === "boolean" ? payload.is_read : undefined
  ...
  const refreshNotifications = useCallback(
    async ({ silent }: { silent?: boolean } = {}) => {
      ...
      try {
        const response = await apiFetchJson<NotificationListResponse>(`/api/notifications?limit=${NOTIFICATION_PAGE_SIZE}&offset=0`)
  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      try {
        ...
        ws = protocols ? new WebSocket(wsTarget.toString(), protocols) : new WebSocket(wsTarget.toString())
      } catch (error) {
  ...
  const handleMarkAsRead = useCallback(
    async (id: string) => {
      ...
      try {
        const response = await apiFetchJson<NotificationUpdateResponse>(`/api/notifications/${encodeURIComponent(id)}/read`, {
          method: "POST",
        })
  ...
  const handleMarkAllAsRead = useCallback(async () => {
    try {
      const response = await apiFetchJson<NotificationUpdateResponse>("/api/notifications/read-all", { method: "POST" })
      await refreshNotifications({ silent: true }).catch(() => undefined)
      if (response && typeof response.unreadCount === "number") {
        setNotificationCount(response.unreadCount)
      } else {
    ...
    <>
      <SidebarContent className={`${isCollapsed ? "p-3" : "p-5"} space-y-6 bg-sidebar border-sidebar-border/50`}>
        ...
        >
          ...
          <AnimatePresence>
            {!isCollapsed && (
              <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} transition={{ delay: 0.1 }}>
                ...
                <p className={`text-xs ${profileError ? "text-red-500" : "text-sidebar-foreground/60"}`}>
                  {profileError ? profileError : profileLoading ? "Loading your workspace..." : resolvedCurrentUser ? `Welcome, ${resolvedCurrentUser.name}` : "Clinical AI Assistant"}
                </p>
                {uiPreferencesError && <p className="text-[11px] text-red-500/90 mt-1">{uiPreferencesError}</p>}
              </motion.div>
            )}
          </AnimatePresence>
        </motion.div>
        ...
        <SidebarGroup>
          <SidebarGroupContent>
            <SidebarMenu className="space-y-0">
              <NavSection items={primaryNavItems} isCollapsed={isCollapsed} currentView={resolvedCurrentView} onNavigate={handleNavigateInternal} />
            </SidebarMenu>
          </SidebarGroupContent>
        </SidebarGroup>
        ...
        <SidebarGroup>
          <SidebarGroupContent>
            <SidebarMenu className="space-y-0">
              <NavSection title="Tools & Resources" items={secondaryNavItems} isCollapsed={isCollapsed} currentView={resolvedCurrentView} onNavigate={handleNavigateInternal} />
            </SidebarMenu>
          </SidebarGroupContent>
        </SidebarGroup>

        {/* Bottom Navigation */}
        <motion.div className={`mt-auto pt-4 ${!isCollapsed ? "border-t border-sidebar-border/60" : ""}`} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}>
          <SidebarGroup>
            <SidebarGroupContent>
              <SidebarMenu className="space-y-0">
                <NavSection
...
export function NavigationSidebar({ currentView, onNavigate, currentUser, userDraftCount }: NavigationSidebarProps) {
  return (
    <Sidebar collapsible="icon" className="border-r border-sidebar-border/50">
      <NavigationSidebarContent currentView={currentView} onNavigate={onNavigate} currentUser={currentUser} userDraftCount={userDraftCount} />
    </Sidebar>
  )
}
